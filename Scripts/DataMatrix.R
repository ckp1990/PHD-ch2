...
# Refactored for sf
# modelgrid=maptools:::read.shape(paste0(shp)) # OLD
# modelpoly=readShapePoly(paste0(shp)) # OLD
modelpoly = st_read(paste0(shp), quiet = TRUE) # NEW: Read as sf object
...
newy=newy2[,,-subsetlist] ## Now, dim(newy) has 13 group size classes, 19 distance classes, 77 transects##
nwalks=nwalks2[-subsetlist]
ntrans=length(nwalks)
ndistwalk=tottrlen[-subsetlist]

### Generate adjacency matrix ###
# poly <- maptools:::Map2poly(modelgrid) # OLD
# adjmatrix=poly2nb(modelpoly) # OLD (often works with sp object)

# NEW: poly2nb works with sf objects directly
adjmatrix=poly2nb(modelpoly) 

### Scaling and centering of landscape level covariates ###
# cov1km=modelgrid$att.data # OLD: sp object slot for data
cov1km = st_drop_geometry(modelpoly) # NEW: extract data.frame from sf
cov1km$LineID=1:nrow(cov1km)

# Scaling
cov1km[names(cov1km) %in% sb2] <- lapply(cov1km[,names(cov1km) %in% sb2], function(x) c(scale(x)))
cov1km_data = cov1km # Rename to be clear it's the data frame
...
newTR[,5]=pmatch(newTR[ ,2], cov1km_data$NUMMER, duplicates=TRUE)
b=newTR[,5]=='NA'
c=which(b=="FALSE")
newTR[-c,5]=1
